...
async def get_tier(tier: str, db: Session = Depends(get_db)):
    try:
        ...
        query1 = db.query(tb_info).filter(
            tb_info.tier == tier, tb_info.position == "TOP"
        )
        query1 = query1.order_by(tb_info.champion_tier.desc())
        query2 = db.query(tb_info).filter(
            tb_info.tier == tier, tb_info.position == "JUNGLE"
        )
        query2 = query2.order_by(tb_info.champion_tier.desc())
        query3 = db.query(tb_info).filter(
            tb_info.tier == tier, tb_info.position == "MIDDLE"
        )
        query3 = query3.order_by(tb_info.champion_tier.desc())
        query4 = db.query(tb_info).filter(
            tb_info.tier == tier, tb_info.position == "BOTTOM"
        )
        query4 = query4.order_by(tb_info.champion_tier.desc())
        query5 = db.query(tb_info).filter(
            tb_info.tier == tier, tb_info.position == "UTILITY"
        )
